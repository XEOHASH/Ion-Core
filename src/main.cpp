...
unsigned int DeltaRetargetingAlgorithm(const INDEX_TYPE pindexLast, const BLOCK_TYPE block, 
      ...
      bnNew = BIGINT_MULTIPLY(bnNew, uint256(nWeightedTimespan));
      bnNew = BIGINT_DIVIDE(bnNew, uint256(nRetargetTimespan));
      ...
      if (nLBTimespan > 0 && nLBTimespan < nLowTimeLimit && BIGINT_GREATER_THAN(bnNew, bnComp)) {
        ...
        if (nLBTimespan < nFloorTimeLimit) {
          SET_COMPACT(bnNew, INDEX_TARGET(pindexLast));
          bnNew = BIGINT_MULTIPLY(bnNew, uint256(95));
          bnNew = BIGINT_DIVIDE(bnNew, uint256(PERCENT_FACTOR));
        } else {
          SET_COMPACT(bnNew, INDEX_TARGET(pindexLast));
        }
      }
      ...
      if ((BLOCK_TIME(block) - INDEX_TIME(pindexLast)) > nLongTimeLimit) {
        ...
        for (int i = 0; i < nNumMissedSteps; ++i) {
          bnNew = BIGINT_MULTIPLY(bnNew, uint256(110));
          bnNew = BIGINT_DIVIDE(bnNew, uint256(PERCENT_FACTOR));
        }
      }
